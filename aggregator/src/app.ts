  ...
  await channel.consume('aggregator', async (msg) => {
    if (msg !== null) {
      ...
      if (routeRaw && responseIndex < routeRaw.route.length) {
        ...
        if (routeRaw?.finalResult && message?.resultStack[routeRaw.finalResult], routeRaw?.finalResult !== "") {
          const sentMessage = {
            correlationId : message.correlationId,
...
,          }
          await channel.sendToQueue("api_gateway_response", Buffer.from(JSON.stringify(successMessage)));
        }
      }
